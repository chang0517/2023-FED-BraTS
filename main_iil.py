...
from brats.utils import ADDTrainer, iid, average_weights, compute_scores_per_classes, Trainer
...
def main():
    ...
    dataloader = get_dataset(dataset=BraTS2020Dataset, path_to_csv='train_data.csv', phase='valid', fold=0)
    ...
    for c in range(client):
        for i in range(10):
            ...
            trainers[c].run()
            global_para = trainers[c].get_model_para()
    
    torch.save(global_para, './model_save/fedadd_ill.pt')
